...
class ModelSelection:
    ...
    def compare_models(train_test):
        ...
        for model_name, select_model in model_list:
            ...
            roc_auc = auc(fpr, tpr)
            fprlist.append(fpr.tolist())
            tprlist.append(tpr.tolist())
            rocaucscore.append(roc_auc)
        ...
        rocgraphlist.append(scn)
        rocgraphlist.append(fprlist)
        rocgraphlist.append(tprlist)
        rocgraphlist.append(sc)
        print("fprlist", fprlist)
        print("tprlist", tprlist)
        print("rocgraphlist", rocgraphlist)
        return scn, sc, {"modelaccuracy": modelaccuracy, "model_roc_auc_score": model_roc_auc_score,
                         "model_recall_list": model_recall_list,
                         "model_precision_list": model_precision_list}, model_cm_list, model_pic_name, rocgraphlist
    ...
    @staticmethod
    def run_tuning(modelname, train_test, reduced_train_test, all_met):
        if modelname == 'logistic_regression':
            ...
            LRparams, tun_par = ModelSelection.run_model(model, params, reduced_train_test)
            ...
            RFCparams, tun_par = ModelSelection.run_model(model, params, reduced_train_test)
            print("Fine Tunned random_forest Model")
            rfc_acc = all_met[0]
            rfc_auc = all_met[1]
            rfc_recall = all_met[2]
            rfc_preci = all_met[3]
            ...
            ABCparams, tun_par = ModelSelection.run_model(model, params, reduced_train_test)
            print("Fine Tunned adaboost Model")
            abc_acc = all_met[0]
            abc_auc = all_met[1]
            abc_recall = all_met[2]
            abc_preci = all_met[3]
